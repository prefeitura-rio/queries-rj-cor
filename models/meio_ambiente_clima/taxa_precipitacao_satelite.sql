...
{{
    config(
        ...
        post_hook='CREATE OR REPLACE TABLE `rj-escritorio-dev.meio_ambiente_clima_staging.taxa_precipitacao_satelite_last_partition` AS (SELECT CURRENT_DATETIME("America/Sao_Paulo") AS data_particao)'
    )
}}
...
FROM `rj-escritorio-dev.meio_ambiente_clima_staging.taxa_precipitacao_satelite`
...
WHERE 
    ...
    data_particao > (SELECT 
                        max(data_particao) 
                    FROM 
                        `rj-escritorio-dev.meio_ambiente_clima_staging.taxa_precipitacao_satelite_last_partition`
                    )
{% endif %}
